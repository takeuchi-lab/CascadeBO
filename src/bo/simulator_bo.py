...
from bo import DecisionResult, DecisionFunc, DiscardFunc, CBORange
...
class OutputStock:
    ...
    output_info: dict = dc.field(default_factory=dict)


@dc.dataclass
class RealCascade:
    bench: list[bm.RealFunction]
    ctrl_dim: list[int]
    each_cost: list[int]
    ...
    def observe_to_end(self, x, minus=False):
        ...
        for i in range(self.n_stage):
            if i == 0:
                tmp_out = [self.bench[0].eval_f(xi) for xi in x_sp[0]]
                if isinstance(tmp_out, tuple):
                    tmp_y = [item[0] for item in tmp_out]
                    info = [item[1] for item in tmp_out]
                    prev_y = np.array(tmp_y).reshape(nx, -1)
                else:
                    tmp_y = tmp_out
                    info = None
                    prev_y = np.array(tmp_y).reshape(nx, -1)
            else:
                concat_x = np.hstack([prev_y, x_sp[i]])
                if info is not None:
                    tmp_y = [self.bench[i].eval_f(xi, item) for xi, item in zip(concat_x, info)]
                else:
                    tmp_y = [self.bench[i].eval_f(xi.reshape(1, -1)) for xi in concat_x]
                info = None
                prev_y = np.array(tmp_y).reshape(nx, -1)
        if minus:
            prev_y *= -1
...
class RealMultioutCascadeBO:
    def __init__(self, cascade: RealCascade, model: CascadeMOGP, method: str, seed: int, f_best: float, f_opt: float,
        ...
        decision_funcs: dict[str, DecisionFunc] = {'ei': self._ei_decision,
                                                   ...
                                                   'ucb_seq_upd': self._ucb_seq_upd_old_decision,
        ...
        self.f_opt = f_opt
        af_opt = MultistartLBFGS(rough_optimizer=LBFGS(ftol=1e-3, gtol=1e-3), strict_optimizer=LBFGS(),
                                 n_parallel=n_thread, seed=self.seed)
        if method == 'ub_upd':
            af_opt = MultistartLBFGS(rough_optimizer=LBFGS(ftol=1e-3, gtol=1e-3), strict_optimizer=LBFGS(),
                                     n_parallel=n_thread, seed=self.seed, n_init_sample=100)
        self.af_optimizer = af_opt
    ...
    def cascade_uncertainty(self, remain_ctrl_param: np.ndarray, obs_stage: int, prev_y=None, with_grad: bool = False):
        if prev_y is not None:
            prev_y = prev_y.flatten()
        cUS = CascadeUncertainty(self.model, lipschitz=self.lipschitz)
        if with_grad:
            x = to_float_tensor(remain_ctrl_param)
            x.requires_grad = True
            if prev_y is None:
                ...
                x_cat = torch.cat([to_float_tensor(prev_y), x])
            x_cat = x_cat.reshape(1, -1)
            us = cUS.evaluate(obs_stage=obs_stage, x=x_cat)
            grad = torch.autograd.grad(us, x)[0]
            return us.detach().clone().numpy().astype(np.float64), grad.detach().clone().numpy().astype(np.float64)
        else:
            nx = remain_ctrl_param.shape[0]
            x = to_float_tensor(remain_ctrl_param)
            if prev_y is None:
            ...
            with torch.no_grad():
                us = cUS.evaluate(obs_stage=obs_stage, x=x_cat)
            return us.clone().numpy().astype(np.float64)

    def _us_optimize(self, obs_stage: int, prev_y=None, additional_point=None):
    ...
    def _ucb_seq_upd_decision(self) -> DecisionResult:
        ...
        if len(self.sorted_stocks) == 0:
            ...
            result_us = self._us_optimize(obs_stage=next_stg, prev_y=None)
            

            ucb_improve = result_ucb.fun - self.target_lower
            if not self.maximize:
                ucb_improve *= -1
            scaled_us = result_us.fun * eta
            if ucb_improve >= scaled_us:
                af = ucb_improve
                batch_x = result_ucb.x
            else:
                print(f'ucb_improve {float(ucb_improve)}  < scaled_us {float(scaled_us)}')
                af = scaled_us
                batch_x = result_us.x
            next_x = batch_x[:self.model.in_dims[next_stg]].reshape(1, -1)
            stock_i = None
            ...
            result_ucb = self._ucb_optimize(obs_stage=next_stg, prev_y=stock.y_value, lcb=not self.maximize,
                                            additional_point=stock.suggested_ctrl_param)
            result_us = self._us_optimize(obs_stage=next_stg, prev_y=stock.y_value)
            ucb_improve = result_ucb.fun - self.target_lower
            scaled_us = result_us.fun * eta
            if ucb_improve >= scaled_us:
                af = ucb_improve
            ...
            next_x = np.hstack([prev_y, batch_x[:self.cascade.ctrl_dim[next_stg]]]).reshape(1, -1)
            stock_i = 0
        nfev = result_ucb.total_nfev + result_us.total_nfev
    ...
    def observe_next(self, fit_model: bool = True, *, enable_memory_profile=False, log_stream=None):
        ...
        if index is not None and self.sorted_stocks[index].output_info:
            tmp_out = self.cascade.bench[next_stg].eval_f(next_x, self.sorted_stocks[index].output_info)
        else:
            tmp_out = self.cascade.bench[next_stg].eval_f(next_x)
        ...
        if next_stg != self.n_stage - 1:
            new_stock = OutputStock(next_stg, next_y, need_update=True, remain_times=self.stock_lifetime,
                                    obtained_iteration=self.iteration, output_info=info,
