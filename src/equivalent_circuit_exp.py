...
def one_exp(seed: int, method: str, max_cycle: int, n_thread: int = 1, n_init: int = 5,
    ...
    f2 = bm.EC_2nd()
    func_list = [f1, f2]
    ctrl_domain = [f1.bounds, f2.bounds[-1:]]
    out_dim = [2, 1]
    ...
    for i in range(cascade.n_stage):
        if i == 0:
            ...
            tmp_y = [cascade.bench[0].eval_f(xi) for xi in init_x_sp[0]]
    ...
    outputscale_bounds = [[(0.01, 1.), (0.01, 0.1)],
                          [(10., 25.)]]
    ...
    print(f'Equivalent Circuit 2stage, HP: optimize (mll)\n')
...
if __name__ == '__main__':
    ...
    result_dir = root_dir / f'equivalent_circuit_cycle{args.max_cycle}'
