...
def plot_sequential(cost_list: list, sus=bool):
    sus_methods = [
                Method('ei_l1', 'EI-based-SUS', 'tab:blue', '-'),
                Method('ei_l1_Dci_intv1_b2.00', 'EI-SUS-R', 'tab:cyan', '-')
                   ]
    ...
    if sus_methods:
        for i in range(len(seq_methods)):
            seq_methods[i].ls = '--'
        main_methods = sus_methods + seq_methods

    for item, cost in it.product(sus_methods, cost_list):
        item.result[cost] = get_result(item, cost)
    ...
    for cost in remain_cost:
        each_cost[cost] = sus_methods[0].result[cost][0]['each_cost']
        converted_cost_hist[cost] = np.r_[0, np.tile(each_cost[cost], plot_cycle).cumsum()]
    ...
    for cost in cost_list:
        ...
        if cost=='same':
            pdf_path = save_dir / f'figure6_sample_path_5_cost_same.pdf'
        if cost=='cons':
            pdf_path = save_dir / f'figure6_sample_path_5_cost_tail_heavy.pdf'
...
if __name__ == '__main__':
    ...
    n_stage = 5
    ctrl_dim = 2
    result_dir = base_dir / f'sample_path_ls3_s{n_stage}d{ctrl_dim}_cycle{max_cycle}/'
    title = 'Sample path (N = 5)'
