...
def plot_best_value(methods: list[Method], cost: str, std_coef=None, i_regret=False, ylim=(23.7, 23.9)):
    fig, ax = plt.subplots()
    ...
    fig_zoom = copy_figure(fig)
    fig.tight_layout()

    fig_zoom.axes[0].set_ylim(ylim)
...
def plot_sequential(cost_list: list, sus=bool):
    sus_methods = [Method('ei_l1', 'EI-SUS(1)', 'tab:blue', '-'),
                  Method('ei_l1000', r'EI-SUS($\infty$)', 'tab:purple', '-'),
                  ]
    seq_methods = []


    main_methods = sus_methods
    ...
    for cost in cost_list:
        ...
        pdf_path = save_dir / f'figure6_solarcell.pdf'

        with PdfPages(pdf_path) as pdf:  
            if mode=='solarcell':
                ylim=(23.7, 23.9)
            figs = plot_best_value(main_methods, cost, ylim=ylim)
            for item in figs:
                pdf.savefig(item)
    return
...
if __name__ == '__main__':
    ...
    n_stage = 3
    result_dir = base_dir / f'solarcell3s_noise_m2_cycle{max_cycle}/'
    title = 'Solar cell (N = 3)'
