...
def one_exp(seed: int, gp_seed: int, setting_name: str, method: str, max_cycle: int, n_thread: int = 1, n_init: int = 5,
            cost_policy='same',
            noise_scale=1e-2,
            length_scale=2,
    ...
    id_str = f'{method}_l{stock_lifetime}{discard_abbr}_seed{gp_seed}{seed:0>2}_{cost_policy}'
    ...
    gp_ls = float(length_scale)
    gp_var = (10 / 2.58) ** 2
    gp_ls_bound = [(gp_ls / 2., 2 * gp_ls)]
    gp_var_bound = [(gp_var / 4., gp_var * 4.)]
    ...
    for i in range(n_stage):
        bounds = [(-10., 10.) for _ in range(ctrl_dim[i])]
        ctrl_domain.append(bounds.copy())
        if i > 0:
            bounds.insert(0, output_range[-1])
        f_i = bm.GPTest(dim=input_dim[i], seed=gp_seed + i * 10, ls=gp_ls, var=gp_var, bounds=bounds)
    ...
    for i in range(cascade.n_stage):
        ...
        gp_i = IndependentMOGP(xt, yt, noise_var=noise_scale ** 2, use_ard=False,
                               lengthscale_bounds=gp_ls_bound, outputscale_bounds=gp_var_bound,
                               prior_mean=0., lengthscale=gp_ls, outputscale=gp_var)
        gp_list.append(gp_i)
    ...
    print(f'sample path: RBF, ls={gp_ls}, var={gp_var}\n')
    ...
    while bo.current_cost < max_cost:
        ...
        if profile_memory:
            ...
            bo.observe_next(fit_model=False)
...
def main():
    one_exp(seed=args.seed, gp_seed=args.gpseed, setting_name=args.setting, method=args.method,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gpseed', '-g', required=True, type=int, help='seed for sample path generation')
    ...
    parser.add_argument('--length_scale', type=int, default=3, help='length scale parameter of GP kernel')
    ...
    result_dir = root_dir / f'sample_path_ls{args.length_scale}_{args.setting}_cycle{args.max_cycle}'
