  ...
  for gpseed in $(seq 0 9); do
    for seed in $(seq 0 1); do
      for method in "${methods[@]}"; do
        echo "${setting}" "${seed}" "${method}" "${cost}" -g "${gpseed}" -t "${n_cpu}" -m ${max_cycle} -l ${reuse} --n_init=${n_init} -d=save_all --root_beta=2 --length_scale=3
      done
    done
  done
}
...
python src/plot/plot_figure3_sample_path_3.py
